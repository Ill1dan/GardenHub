    ...
    basePlants.forEach(base => {
        base.varieties.forEach(variety => {
            plants.push({
                ...
                diseases: base.problems.map(p => p.name).join(", "), // Summary string for Plant model compatibility
                scientific_name: `${base.name} ${variety.toLowerCase()}`, // Mock scientific name
